...
class UserController extends Controller
{
    ...
    public function signup(Request $request)
    {
        ...
        $User->child_account_type = 'businessCustomer';
    ...
    public function verify_sign_otp(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $TempUser = TempUsers::when(!empty($request->username), function ($query) use ($request) {
        ...
        if (!empty($TempUser)) {
            ...
            if ($TempUser->vcode == $request->vcode) {
                ...
                if (strtotime($TempUser->vcode_expiry) >= strtotime(date('Y-m-d H:i:s'))) {
                    ...
                    $User->privacy_policy_date = Carbon::now();
                    $User->license_date = Carbon::now();
                    if (isset($TempUser->user_group_id)) {
                        $User->user_group_id = $TempUser->user_group_id;
                    }
                    $User->first_login = 0;
                    $User->parent_id = $TempUser->parent_id;
                    if (isset($TempUser->child_account_type)) {
                        $User->child_account_type = $TempUser->child_account_type;
                    }
                    $User->save();
    ...
    public function send_pincode_with_username_email(Request $request)
    {
        ...
        if (current_method() == 'send_otp') {
            $User = TempUsers::where('email', $request->email)->orWhere('username', $request->email)->orderBy('id', 'DESC')->first();
            if ($User) {
                $User = User::where('email', $request->email)->orWhere('username', $request->email)->first();
            } else {
                $User = $User->first();
                $MainUser = User::where('email', $request->email)->orWhere('username', $request->email)->first();
                if ($MainUser) {
            ...
            $User = User::where('email', $request->email)->orWhere('username', $request->email)->first();
            $TempUser = TempUsers::where('email', $request->email)->orWhere('username', $request->email)->orderBy('id', 'DESC')->first();
            if ($TempUser) {
        ...
        if (!empty($User)) {
            ...
            if (current_method() != 'send_otp' && $User->provider != '') {
                ...
                if (current_method() == 'send_otp') {
                 ...
                 // for existing email child
                if($User->child_account_type == 'withExistingEmail'){
                    $Parent = User::where('id', $User->parent_id)->select('email')->first();
                    $email = $Parent->email;
                }else{
                    $email = $User->email;
                }
        ...
        } elseif(!empty($TempUser)){

            if (current_method() != 'send_otp' && $TempUser->provider != '') {
                $data['success'] = FALSE;
                $data['message'] = 'Invalid email.';
                $data['data'] = (object)[];
                return response()->json($data, 400);
                ...
                if (current_method() == 'send_otp') {
                    $html = 'Hi ' . $TempUser->username . ',<br><br> We received a request for OTP. <br><br>Your OTP is: ' . $TempUser->vcode . '';
                } else {
                    $html = 'Hi ' . $TempUser->username . ',<br><br> We received a request to reset your password. <br><br>Your code is: ' . $TempUser->vcode . '';
                }

                 // for existing email child
                if($TempUser->child_account_type == 'withExistingEmail'){
                    $Parent = User::where('id', $TempUser->parent_id)->select('email')->first();
                    $email = $Parent->email;
                }else{
                ...
                $emailInfo["email"] = $email;
                $emailInfo["subject"] =  (current_method() == 'send_otp') ? config("app.name", "") . ': OTP' : config("app.name", "") . ': Reset Password OTP';
                $emailInfo["otp"] = $TempUser->vcode;
                $emailInfo["username"] = $TempUser->username;
                ...
                if (strtolower(config("app.name", "")) != 'addmee') {
                    ...
                    $body = [
                        'Messages' => [
                            [
                                'From' => ['Email' => "no-reply@addmee.de", 'Name' => config("app.name", "")],
                                'To' => [['Email' => $email, 'Name' => $TempUser->username]],
                                'Subject' => $subject,
                                'TextPart' => $subject,
                                'HTMLPart' => $html,
                                'CustomID' => config("app.name", "")
                ...
                $template = anyTemplateAssigned($TempUser->id);
                $TempUser = UserObj($TempUser, 0, $template);
    ...
    public function send_pincode(Request $request)
    {
        ...
        $otp_code = rand(111111, 999999);
        if (current_method() == 'send_otp') {
            $User = TempUsers::where('email', $request->email)->where('child_account_type','!=','withExistingEmail')->orderBy('id', 'DESC')->first();
            if ($User) {
                $User = User::where('email', $request->email)->where('child_account_type','!=','withExistingEmail')->first();
            } else {
                $User = $User->first();
                $MainUser = User::where('email', $request->email)->where('child_account_type','!=','withExistingEmail')->first();
                if ($MainUser) {
                    $MainUser = $MainUser->first();
                    $MainUser->vcode_expiry = date('Y-m-d H:i:s', strtotime('+1 day'));
                    $MainUser->vcode = $otp_code;
            ...
            $User = User::where('email', $request->email)->where('child_account_type','!=','withExistingEmail')->first();
            $TempUser = TempUsers::where('email', $request->email)->where('child_account_type','!=','withExistingEmail')->orderBy('id', 'DESC')->first();
            if ($TempUser) {
               // $TempUser = $TempUser->first();
                $TempUser->vcode_expiry = date('Y-m-d H:i:s', strtotime('+1 day'));
                $TempUser->vcode = $otp_code;
        ...
        if (!empty($User)) {
            ...
            if (current_method() != 'send_otp' && $TempUser->provider != '') {
                ...
                //sendgrid_api($html, env("MAIL_FROM_NAME", "").' Code is: '.$User->vcode, $User->email, $User->name);
                $emailInfo["email"] = $TempUser->email;
                $emailInfo["subject"] =  (current_method() == 'send_otp') ? config("app.name", "") . ': OTP' : config("app.name", "") . ': Reset Password OTP';
                $emailInfo["otp"] = $TempUser->vcode;
                $emailInfo["username"] = $TempUser->username;
                ...
                if (strtolower(config("app.name", "")) != 'addmee') {
                    ...
                    $body = [
                        'Messages' => [
                            [
                                'From' => ['Email' => "no-reply@addmee.de", 'Name' => config("app.name", "")],
                                'To' => [['Email' => $TempUser->email, 'Name' => $TempUser->username]],
                                'Subject' => $subject,
                                'TextPart' => $subject,
                                'HTMLPart' => $html,
                                'CustomID' => config("app.name", "")
    ...
    public function send_otp_with_username(Request $request)
    {
        $validations['email'] = 'required|string';
        $validator = Validator::make($request->all(), $validations);
        ...
        if (!empty($User)) {
            ...
            if (current_method() != 'send_otp' && $User->provider != '') {
                ...
                if (current_method() == 'send_otp') {
                    $html = 'Hi ' . $User->username . ',<br><br> We have received a request to resend the OTP. <br><br>Your code is: ' . $User->vcode . '';
                } else {
                    $html = 'Hi ' . $User->username . ',<br><br> We have received a request to resend the OTP. <br><br>Your code is: ' . $User->vcode . '';
                }
                ...
                //sendgrid_api($html, env("MAIL_FROM_NAME", "").' Code is: '.$User->vcode, $User->email, $User->name);
                $emailInfo["email"] = $email;
                $emailInfo["subject"] =  (current_method() == 'send_otp') ? config("app.name", "") . ': OTP' : config("app.name", "") . ': Resend OTP Request';
                $emailInfo["otp"] = $User->vcode;
                $emailInfo["username"] = $User->username;
                ...
                if (strtolower(config("app.name", "")) != 'addmee') {
                    ...
                    $mj = new \Mailjet\Client('4133c35d76d4c148cad9d63efa8ed0cc', 'c0e0cc12628811dfea4a53ccd3f95f7f', true, ['version' => 'v3.1']);
                    $subject =  (current_method() == 'send_otp') ? config("app.name", "") . ': OTP' : config("app.name", "") . ': Resend OTP Request';
                    $body = [
                        'Messages' => [
                            [
                                'From' => ['Email' => "no-reply@addmee.de", 'Name' => config("app.name", "")],
            ...
            if (current_method() != 'send_otp' && $TempUser->provider != '') {
                ...
                if (current_method() == 'send_otp') {
                    $html = 'Hi ' . $TempUser->username . ',<br><br> We have received a request to resend the OTP. <br><br>Your code is: ' . $TempUser->vcode . '';
                } else {
                    $html = 'Hi ' . $TempUser->username . ',<br><br> We have received a request to resend the OTP. <br><br>Your code is: ' . $TempUser->vcode . '';
                }
    ...
    public function send_otp(Request $request)
    {
        ...
        $otp_code = rand(111111, 999999);
        if (current_method() == 'send_otp') {
            $User = TempUsers::where('email', $request->email)->orderBy('id', 'DESC')->first();
            if ($User) {
                $User = User::where('email', $request->email)->first();
            } else {
                $User = $User->first();
    ...
    public function verify_pincode(Request $request)
    {
        ...
        $User = User::where('email', $request->email)->where('child_account_type','!=','withExistingEmail');
        if ($User->count() == 0) {
            $User = TempUsers::where('email', $request->email)->where('child_account_type','!=','withExistingEmail')->orderBy('id', 'DESC');
        }
    ...
    public function reset_password_with_username_email(Request $request)
    {
        $validations['email'] = 'required|string';
        $validations['password'] = 'required|string|confirmed|min:6';
        ...
        if ($User->provider != '') {
            ...
            $body = [
                'Messages' => [
                    [
                        'From' => ['Email' => "no-reply@addmee.de", 'Name' => config("app.name", "")],
                        'To' => [['Email' => $email, 'Name' => $User->first_name . ' ' . $User->last_name]],
                        'Subject' => $subject,
                        'TextPart' => $subject,
                        'HTMLPart' => $html,
                        'CustomID' => config("app.name", "")
    ...
    public function loginWithUsernameEmail(Request $request)
    {
        ...
        if (current_method() == 'customer-login') {
            $User = User::where('email', $request->email)->orWhere('username',$request->email)->whereIn('user_group_id', [2, 3, 7, 8, 9, 10])->first();
    ...
    public function login(Request $request)
    {
        ...
        if (current_method() == 'customer-login') {
            $User = User::where('email', $request->email)->whereIn('user_group_id', [2, 3, 7, 8, 9, 10])->first();
            if ($User && $User->user_group_id == 2) {
                $BusinessUser = BusinessUser::where('user_id', $User->id)->where('user_role', 'admin');
                if ($BusinessUser->count() == 0) {
                    $data['success'] = FALSE;
        ...
        if ($User) {
            ...
            if (Hash::check($request->password, $User->password)) {
                ...
                $ChildAccounts = User::where('parent_id', $User->id)
                ->select('id', 'first_name', 'last_name', 'designation', 'company_name', 'company_friendly_name', 'company_address', 'company_logo', 'email', 'username', 'dob', 'gender', 'user_group_id', 'logo', 'bio', 'banner', 'is_pro', 'is_public', 'profile_view', 'status', 'provider', 'provider_id', 'device_id', 'device_type', 'platform', 'email_verified_at', 'last_login', 'subscription_date', 'subscription_expires_on', 'open_direct', 'fcm_token', 'allow_data_usage', 'privacy_policy_date', 'license_date', 'first_login', 'forced_reset', 'note_description', 'note_visible_from', 'note_visible_to', 'created_by', 'updated_by', 'created_at', 'updated_at', 'role', 'parent_id','child_account_type', 'switch_account_status')
                ->get();
                if ($ChildAccounts->count() > 0) {
                    $ChildAccounts = userChildsObj($ChildAccounts, $template);
                }else{
    ...
    public function profile(Request $request)
    {
        ...
        if ($User) {
            ...
            $data['data'] = array('user' => $User, 'child_accounts' => $ChildAccounts, 'isExistingSubsValid' => $isExistingSubsValid, 'is_editable' => $is_editable, 'settings' => $UserSettings, 'anyTemplateAssigned' => $anyTemplateAssigned, 'template' => (object)$_template,'businessCustomer' => $businessCustomer,);
            return response()->json($data, 201);
        } else {
    ...
    public function delete_account(Request $request)
    {
        ...
        if (!empty($User)) {
            ...
            if ($token->provider != '' || $token->provider != null || $User->child_account_type == 'withExistingEmail') {
                ...
                $childAccounts = User::where('parent_id', $token->id)->get();
                foreach ($childAccounts as $child) {
                    $childAccount = User::where('parent_id', $child->parent_id)->first();

                    BusinessInfo::where('user_id', $child->id)->delete();
                    ContactCard::where('user_id', $child->id)->delete();
                    CustomerProfile::where('user_id', $child->id)->delete();
                    TapsViews::where('user_id', $child->id)->delete();
                    UserNote::where('user_id', $child->id)->delete();
                    $child->delete();
                    $childAccount->delete();
                }
            ...
            if (strtolower(config("app.name", "")) != 'addmee') {
                ...
                Mail::send([], [], function ($message) use ($html, $User,$email) {
                    $message
                        ->to($email)
        ...
        if ($User->vcode == $request->vcode) {
            ...
            if (strtotime($User->vcode_expiry) >= strtotime(date('Y-m-d H:i:s'))) {
                ...
                if ($User->id == $token->id) {
                    ...
                    $childAccounts = TempUsers::where('parent_id', $token->id)->get();
                    foreach ($childAccounts as $child) {
                    $childAccount = User::where('parent_id', $child->parent_id)->first();
                ...
                if (!empty($users)) {
                    ...
                    foreach ($users as $user) {
                        ...
                        if ($updated_members->count() != 0) {
                            ...
                            unset($updated_member->vcode, $updated_member->vcode_expiry, $updated_member->access_token
                            ,$updated_member->role,$updated_member->fcm_token,$updated_member->name,$updated_member->name
                            ,$updated_member->designation, $updated_member->dob, $updated_member->gender, $updated_member->custom_gender,
                            $updated_member->user_group_id, $updated_member->custom_gender, $updated_member->custom_gender,
                            $updated_member->company_friendly_name, $updated_member->is_public, $updated_member->is_public,
                            $updated_member->provider, $updated_member->provider_id, $updated_member->platform,
                            $updated_member->email_verified_at, $updated_member->last_login, $updated_member->subscription_date,
                            $updated_member->subscription_expires_on, $updated_member->last_login, $updated_member->forced_reset,$updated_member->profile_view,
                            $updated_member->note_description, $updated_member->note_visible_from, $updated_member->note_visible_to, $updated_member->updated_by);
    ...
    try {
        ...
        $userEmail = "rahmanullahkhan2222@gmail.com";
        $subject = "testing email";
        $body = "testing email otp";

        sendOutlookEmail($userEmail, $subject, $body);
